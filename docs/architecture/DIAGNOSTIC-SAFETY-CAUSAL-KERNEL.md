...
Status: implemented in source for MEET 4.16; build, migration and hardware conformance remain unexecuted until explicitly authorized.
...
A non-empty `capabilityPackId` never authenticates itself. `ActiveDiagnosticCapabilityRegistry` is the production trust boundary and currently authorizes no pack until signature verification, revocation and vehicle/ECU applicability are implemented. Consequently all built-in generic active-test definitions remain safely non-executable.
...
Remote diagnostic telemetry defaults to `DISABLED`. Settings expose explicit local-only, anonymous/redacted and consented-redacted modes. VIN is hashed; VIN-like strings, MAC addresses and IPv4 addresses are redacted before permitted remote diagnostics. Local evidence retention and remote telemetry consent are separate policies.
...
- encrypted-at-rest diagnostic evidence still needs a reviewed key-management and database-migration design;
- Room schema exports for versions 51 and 52 must be generated by the authorized build toolchain and reviewed before release;
...
- Room 51→52 migration test with real and adversarial legacy databases;
