...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Public read oem_pids') THEN
    CREATE POLICY "Public read oem_pids" ON oem_pids FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Public read dtc_definitions') THEN
    CREATE POLICY "Public read dtc_definitions" ON dtc_definitions FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Public read service_resets') THEN
    CREATE POLICY "Public read service_resets" ON service_resets FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users manage own scans and Mechanics view all') THEN
    CREATE POLICY "Users manage own scans and Mechanics view all" ON scan_sessions FOR ALL USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users manage own vehicles and Mechanics view all') THEN
