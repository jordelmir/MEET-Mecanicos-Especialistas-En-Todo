...
class SpeechService @Inject constructor(
    ...
    fun startListeningToEvents() {
        if (job != null) return
        Log.i("SpeechService", "Starting Elysium Vanguard AI OS Copilot SpeechService event listener...")
        job = scope.launch {
    ...
    fun stopListeningToEvents() {
        job?.cancel()
        job = null
        Log.i("SpeechService", "Stopped Elysium Vanguard AI OS Copilot SpeechService event listener.")
    }
