...
ROUTE_ALIASES = {
    ...
    "report": "reports",
    "reportes": "reports",
    "inspection": "inspection_session/hyundai_accent_2005",
    "inspection_session": "inspection_session/hyundai_accent_2005",
    "history": "vehicle_history/hyundai_accent_2005",
    "historial": "vehicle_history/hyundai_accent_2005",
    "vehicle_history": "vehicle_history/hyundai_accent_2005"
}
...
def resolve_route(route_query):
    ...
    if base == "universal" and len(parts) > 1:
        return f"universal_activity/{parts[1]}"
    if base in ["inspection", "inspection_session"] and len(parts) > 1:
        return f"inspection_session/{parts[1]}"
    if base in ["history", "vehicle_history", "historial"] and len(parts) > 1:
        return f"vehicle_history/{parts[1]}"
...
def topup_flow(amount=10000, reference=None):
    ...
    # Close soft keyboard so dialog buttons are fully visible
    run_adb(["shell", "input", "keyevent", "4"])
    time.sleep(0.6)
    
    ok, msg = tap_element("ENVIAR Y NOTIFICAR AL TRUST CENTER")
    if not ok:
        # Try scrolling dialog down or tapping by partial match
        ok, msg = tap_element("NOTIFICAR")
...
def run_test_all_verticals():
    ...
    print("\n[6/9] Navegando a Sesión de Inspección y Reportes Certificados V2...")
    nav("inspection_session/hyundai_accent_2005")
    time.sleep(2.0)
    take_screenshot(f"{out_dir}/10_inspection_session_prescan.png")
    
    print("\n[7/9] Navegando a Historial Técnico Vehicular y Cadena Criptográfica...")
    nav("vehicle_history/hyundai_accent_2005")
    time.sleep(2.0)
    take_screenshot(f"{out_dir}/11_historial_vehicular_cadena.png")
    
    print("\n[8/9] Realizando prueba de recarga SINPE y validación en Trust Center...")
    ref_test = f"SINPE-AUDIT-{int(time.time())}"
    topup_flow(amount=10000, reference=ref_test)
    time.sleep(1.5)
    take_screenshot(f"{out_dir}/12_topup_submitted_pending.png")
    
    nav("trust_center")
    time.sleep(2.0)
    take_screenshot(f"{out_dir}/13_trust_center_queue.png")
    approve_topup()
    time.sleep(1.5)
    take_screenshot(f"{out_dir}/14_trust_center_approved.png")
    
    print("\n[9/9] Verificando acreditación de saldo en cabina...")
    nav("tow_truck")
    time.sleep(1.5)
    take_screenshot(f"{out_dir}/15_grua_balance_updated.png")
